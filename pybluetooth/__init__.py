...
class HCIThread(RxThread):
    ...
    def cmd_inquiry(self):
        self.send_cmd(HCI_Cmd_Inquiry())
    def cmd_inquiry_cancel(self):
        self.send_cmd(HCI_Cmd_Inquiry_Cancel())
    # end daylong
...
class BTStack(object):
    ...
    def start_inquiry(self):
        #assert self.is_scannning_enabled is False
        self.hci.cmd_inquiry()
        self.is_inquiry_enabled = True

    def stop_inquiry(self):
        #assert self.is_scannning_enabled is True
        self.hci.cmd_inquiry_cancel()
        self.is_inquiry_enabled = False
