...
class BTStackSynchronousUtils(object):
    ...
    def inquiry(self, duration_secs=5.0):
        LOG.debug("BTStackSynchronousUtils inquiry()")
        inq_report_queue = Queue.Queue()

        def inq_packet_filter(packet):
            #return packet.getlayer(HCI_LE_Meta_Advertising_Report) is not None
            return True
        self.b.hci.add_packet_queue(inq_packet_filter, inq_report_queue)
        self.b.start_inquiry()
        time.sleep(duration_secs)
        self.b.stop_inquiry()
        self.b.hci.remove_packet_queue(inq_report_queue)
